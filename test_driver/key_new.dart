    ...
    () {
        ...
        () async {
          await driver.runUnsynchronized(
            () async {
              ...
              await driver.tap(find.text('Jumpcoin Testnet'));
              await driver.tap(find.text('Jumpcoin Testnet')); //tap into wallet
      ...
      test(
        ...
        () async {
          ...
          await driver.runUnsynchronized(
            () async {
              await driver.tap(find.byValueKey('newWalletIconButton'));
              await driver.tap(find.text('Jumpcoin'));
              await driver.tap(find.text('Jumpcoin')); //tap into wallet
              expect(await driver.getText(find.text('connected')), 'connected');
            },
          );
        },
      ...
      test('create new peercoin mainnet watch-only wallet and delete it again',
        ...
        await driver.runUnsynchronized(() async {
          ...
          await driver
              .tap(find.text('Jumpcoin')); //create peercoin watch-only wallet
          await driver
              .tap(find.text('Jumpcoin 2')); //tap into watch-only wallet
        });
        ...
        //expect to not find "Jumpcoin 2"
        await driver.runUnsynchronized(
          () async {
            try {
              await driver.getText(find.text('Jumpcoin 2'));
              fail('Expected to not find "Jumpcoin 2"');
            } catch (e) {
              // Expected not to find "Jumpcoin 2", so we swallow the exception
            }
          },
        );
      });
